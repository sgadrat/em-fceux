...
emcc -D 'INPUT_PRE=inputs : {' -D 'INPUT_POST=},' -D 'INPUT(i_,dk_,dg_,e_,t_)=i_ : [ dk_, dg_, t_ ],' -D 'INPUT_HIDDEN(i_,dk_,dg_,e_,t_)=' -E $input_inc -P >> $config_js
...
python2 -c "import os; print 'FCEUX_JS_SIZE : %s,\n};' % (os.stat('$fceux_js').st_size)" >> $config_js
